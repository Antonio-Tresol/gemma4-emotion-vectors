...
chipped rather than done inline. (8) The BASE-READER row of the Q3 factorial (user request):
registered before scoring with a named expectation (E9 predicts the corpus bank recovers on
base) and two caveats (reader and probe lineage move together; no control subtraction exists
for this arm), base-arm calibration generated by the same no-outcome-reads script, then the
identical registered G/R1 reads run on the existing base substrate — no new inference needed.
(9) Q1.H1.E3 (user question): what occupies instruct PC1/PC2 after the valence demotion —
registered four reads (VAD three-column correlations incl. dominance, extremes, cross-model
alignment, story-length nuisance), ran laptop-side from the public means files; exhibit added as
notebook 02 section 8, recomputed in-cell on the post-fix bundles (instrument-robust: 0.83 vs
...
an instruct-reader property and foreshadowing pickup does not require the reader to be the
author. On E3 I expected it-PC1 to be some recognizable covariate; it is NEW near-orthogonal
structure (first principal angle 85.8 degrees, score corr <= 0.19 with every base top-5 PC, no
VAD correlate, affectively incoherent extremes, appears at layer 12, grows to 42% of variance) —
identity unknown. The circumplex plane itself survived intact one slot down: it-PC3 IS base-PC1
(score corr 0.91, valence+dominance bundle) and it-PC2 is base-PC2's arousal axis — but the
story-length nuisance read caught that it-PC2 is more story length (r=+0.69) than arousal
(-0.37), a corpus-composition confound instruction tuning amplified. On E4 two of three
predictions fired (removing the instruct top PC doubles mid-to-late RSA coherence 0.32 -> 0.54
while the same ablation only hurts base; the instruct top component breaks identity exactly at
the 24-30 RSA block boundary, so E3's one inserted axis is really TWO successive axes) and one
failed informatively: unablated instruct-late matches base-late at only 0.29 — but ablating the
...
identify what it-PC1 encodes (candidates to design against: response-preparation/style variance,
formatting structure — needs new evidence, not more correlations with the same 171 points);
notebook 02 exhibit for the E3 answer; repo-wide check.sh gate cleanup (chipped task).
