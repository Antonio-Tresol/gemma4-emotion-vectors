"""Sections 8-9: valence displacement and RSA fragmentation.

The geometry report's sections 8 (what displaced valence, TREE Q1.H1.E3) and
9 (why the instruct RSA fragments, TREE Q1.H1.E4) are load-call-show over
this module. Each section builder returns ``(figure, stats)`` where
``stats["lines"]`` holds every line the notebook prints, so the printed
record and the figure come from one computation.

Number-identity contract: the computations here are line-for-line ports of
the formerly inline notebook cells (commit 9dd6e30), so re-running the
...
from ._context import LATE_BAND, MID_BAND, GeometryContext, nrc_vad_lexicon_path


def load_dominance(lexicon_path: Path | None = None) -> dict[str, float]:
    ...
    section uses, so it is parsed here. Defaults to the checked-in lexicon.
    """
    if lexicon_path is None:
        lexicon_path = nrc_vad_lexicon_path()
