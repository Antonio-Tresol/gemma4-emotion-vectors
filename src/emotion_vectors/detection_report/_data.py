...
* Sections 3 to 6 score probe sets that only ever extracted the 12 battery
...
# Ways of turning a prompt's token activations into one vector. The label
# names what the readout DOES, so a figure legend never shows a bare key.
...
# The two batteries, and the ROLE each one plays in the registered rule.
BATTERY_ROLES = {
    "scenario": "paper battery: picks the best cell",
    "heldout": "held-out battery: confirms it",
}
BATTERY_SHORT = {"scenario": "paper", "heldout": "held-out"}
...
@dataclass
class ModelSweep:
    """One model's probe bundle plus its swept battery and template prompts.
    ...
    the 12 battery emotions, in battery order), ``battery_rows`` (prompt row
    indices per battery), ``prompts`` (the swept prompt records).
    """
    ...
    battery_rows: dict[str, list[int]]
    prompts: list[dict[str, Any]]
    _activations: Any = None  # the open npz; sliced lazily, cached per readout
    _cache: dict[str, np.ndarray] = field(default_factory=dict)

    def activations(self, fmt: str, readout: str) -> Float[np.ndarray, "prompts layers d_model"]:
        """Every swept prompt's activation under one (format, readout), float32.
    ...
    def battery_activations(
        self, fmt: str, readout: str, kind: str, layer_pos: int
    ) -> Float[np.ndarray, "scenarios d_model"]:
        """One battery's 12 scenario activations at one layer."""
        acts = self.activations(fmt, readout)
        return np.stack([acts[row, layer_pos] for row in self.battery_rows[kind]])

    def probe_means(self, layer_pos: int) -> Float[np.ndarray, "battery_emotions d_model"]:
        """The 12 battery probes at one layer, in battery order."""
...
class SweepContext:
    """Both models' sweeps, sharing one layer grid and one battery order.

    ``models[label]`` is that model's :class:`ModelSweep`; ``layers`` is the
    shared layer grid; ``probe_order`` is the 12 battery target emotions in
    scenario order, which is the row order of every probe matrix here.
    """
...
def _load_model_sweep(
    ...
    for kind, rows in battery_rows.items():
        if len(rows) != N_SCENARIOS:
            raise ValueError(
                f"{label}: {kind} battery has {len(rows)} prompts, expected {N_SCENARIOS}"
            )
    return ModelSweep(
        label=label,
...
def battery_counts(
    model: ModelSweep,
    probe_means: Float[np.ndarray, "battery_emotions d_model"],
    fmt: str,
    readout: str,
    layer_pos: int,
    ...
    The registered statistic, evaluated on both batteries at once.
    ...
    center on the 12 probes themselves for sections 3 to 6.
    """
    counts = [
        score_battery(
            probe_means,
            model.battery_activations(fmt, readout, kind, layer_pos),
