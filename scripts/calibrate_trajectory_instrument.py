...
def sample_ids(manifest: str, n: int) -> list[str]:
    ...
    seq = [
        r
        for r in rows
        if r["mode"] == "SEQUENTIAL" and not r["error"] and len(r["phase_token_starts"]) == 3
    ]
...
def load_stories(
    repo: str, ids: list[str], rand_idx: list[int], shard_dir: str | None
) -> list[dict]:
    if shard_dir:
        ...
        with ThreadPoolExecutor(16) as ex:
            paths = list(
                ex.map(
                    lambda sid: hf_hub_download(repo, f"shards/{sid}.npz", repo_type="dataset"), ids
                )
            )
    stories = []
    for p in paths:
        ...
        stories.append(
            dict(
                cos=(
                    d["dots"][:, :, rand_idx].astype(np.float32)
                    / d["norms"].astype(np.float32)[:, :, None]
                ),  # (T, 6, 24)
...
def norm_drift(stories: list[dict]) -> dict[str, object]:
    out = {}
    for li, layer in enumerate(LAYERS):
        r = [
            s["norms"][int(0.75 * s["T"]) :, li].mean()
            / s["norms"][: int(0.25 * s["T"]), li].mean()
            for s in stories
        ]
...
def random_probe_stats(stories: list[dict]) -> dict[str, object]:
    stats = {}
    for li, layer in enumerate(LAYERS):
        ...
        for s in stories:
            ...
            for p in range(len(bounds) - 1):
                ...
                x0 = x[:-1] - x[:-1].mean(0)
                x1 = x[1:] - x[1:].mean(0)
                ar1.append((x0 * x1).sum() / np.sqrt((x0**2).sum() * (x1**2).sum()))
        stats[str(layer)] = dict(
            cos_sd=float(np.mean(sd_all)),
            drift_absrho_med=float(np.median(rho)),
            drift_absrho_p90=float(np.quantile(rho, 0.9)),
...
def null_reads(stories: list[dict]) -> dict[str, object]:
    nulls = {}
    ...
    for li, layer in enumerate(LAYERS):
        ...
        for s in stories:
            ...
            for b in s["starts"][1:]:
                ...
                anticip.append(float(c[b - W : b, jin].mean() - c[b - 2 * W : b - W, jin].mean()))
        ...
        nulls[str(layer)] = dict(
            anticip=dict(
                mean=float(a.mean()), sd=float(a.std()), p95=float(np.quantile(a, 0.95)), n=len(a)
            ),
...
def _fit_step_vs_ramp(
    y: Float[np.ndarray, "window"], tgrid: Float[np.ndarray, "window"]
    ...
    for name, widths in [("step", [1e-6]), ("ramp", RAMP_WIDTHS)]:
        ...
        for c in range(-8, 9, 2):
            for w in widths:
                f = 1 / (1 + np.exp(np.clip(-(tgrid - c) / max(w / 4.6, 1e-6), -60, 60)))
...
def _gen_ar1(
    n: int, length: int, sigma: float, phi: float, rng: np.random.Generator
) -> Float[np.ndarray, "draws window"]:
...
def decidability_sims(sigma: float, phi: float, nsim: int) -> dict[str, object]:
        ...
        for width in [8, 16, 24]:
            ...
            for i in range(nsim):
                x[i] += (
                    amp_sd
                    * sigma
                    / (1 + np.exp(np.clip(-(tgrid - jitters[i]) / (width / 4.6), -60, 60)))
                )
            wins, widths = 0, []
            for y in x:
                bs, br, w = _fit_step_vs_ramp(y, tgrid)
...
def main() -> None:
    ...
    ap.add_argument(
        "--shard-dir", default=None, help="reuse local shard dir instead of downloading"
    )
