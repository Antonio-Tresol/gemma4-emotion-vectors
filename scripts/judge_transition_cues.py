...
def call_judge(model: str, system: str, user: str, key: str,
               retries: int = 3) -> dict:
    ...
    for sid, r, p in parsed:
        ...
        if task == "cue":
            for ti in (1, 2):  # transitions INTO phases 1 and 2
                units.append(dict(
                    story_id=sid, unit=f"tr{ti}", emotion=p.phase_emotions[ti],
                    text=p.clean_text, boundary_char=p.phase_char_starts[ti],
                    category=r["category"]))
        else:  # display: every phase is a unit
            for pi in range(3):
                units.append(dict(
                    story_id=sid, unit=f"ph{pi}", emotion=p.phase_emotions[pi],
...
def run_task(args: argparse.Namespace, key: str) -> None:
    ...
    out = Path(args.out or
               f"results/judge_{args.task}_{args.model.split('/')[-1].split(':')[0]}.jsonl")
    done = set()
    if out.exists():
        done = {(json.loads(l)["story_id"], json.loads(l)["unit"])
                for l in open(out)}
    todo = [u for u in units if (u["story_id"], u["unit"]) not in done]
    print(f"{args.task}: {len(units)} units, {len(done)} done, {len(todo)} to run "
          f"-> {out}")
    ...
    def work(u: dict) -> dict:
        ...
        if "error" in res:
            ...
            row["cue_minus_boundary"] = (None if row["cue_char"] < 0
                                         else row["cue_char"] - u["boundary_char"])
        else:
    ...
    with ThreadPoolExecutor(args.workers) as ex, open(out, "a") as fh:
        ...
        for i, row in enumerate(ex.map(work, todo)):
            ...
            if n_err > max(10, 0.2 * (i + 1)):
                raise SystemExit(f"error rate too high ({n_err}/{i + 1}) — "
                                 "check model id / key / rate limits")
    print(f"done: {len(todo)} rows, {n_err} errors")


def agreement(file_a: str, file_b: str) -> None:
    def load(f: str) -> dict:
        return {(r["story_id"], r["unit"]): r
                for r in map(json.loads, open(f)) if "error" not in r}
    ...
    if "rating" in a[keys[0]]:
        ...
        print(f"display agreement: {same}/{len(keys)} exact "
              f"({same / len(keys):.2f})")
        return
    ...
    import statistics
    print(f"cue agreement over {len(keys)} units: none-flag disagreements "
