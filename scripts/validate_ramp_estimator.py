...
from emotion_vectors.ramp_fit import (  # noqa: E402
    WIDTH_GRID, logistic_curve, profile_width, transition_sse_matrix, verdict)
...
def gen_condition(true_width: float, amp_sd: float, sigma: float, phi: float,
                  rng: np.random.Generator, frac_null: float = 0.0) -> list[np.ndarray]:
...
def gen_gated_mixture(true_width: float, amp_sd: float, sigma: float, phi: float,
                      rng: np.random.Generator, frac_null: float) -> tuple[list, list]:
...
def main() -> None:
    ...
    for true_w, amp, frac_null in conditions:
        ...
        cells[name] = dict(true_width=true_w, amp_sd=amp, frac_null=frac_null,
                           **fit, passes=bool(ok))
        print(name, "->", fit["width"], fit["ci95"], fit["verdict"],
              "PASS" if ok else "FAIL")

    # amended procedure: amplitude-gate (>= 2 sigma on the flank statistic),
    # then fit the retained subset — the mixture rescue
    for true_w, amp, frac_null, tag in [(16.0, 2.0, 0.5, "w16_amp2sd_mix_gated"),
        ...
        if true_w == 0.0:
            ...
            ok = 0.5 * true_w <= fit["width"] <= 1.5 * true_w
        cells[tag] = dict(true_width=true_w, amp_sd=amp, frac_null=frac_null,
                          **fit, passes=bool(ok))
        print(tag, "->", fit["width"], fit["ci95"], fit["verdict"],
              f"retained {fit['retained_frac']:.2f}", "PASS" if ok else "FAIL")

    out = dict(sigma=sigma, phi=phi, window=WINDOW, n_transitions=N_TRANS,
               jitter_sd=JITTER_SD, n_boot=N_BOOT, width_grid=list(WIDTH_GRID),
               cells=cells,
